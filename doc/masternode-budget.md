...
NeoBitcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<neobitcoin_address\> \<monthly_payment_neobitcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ NBTC, _12_ times over the course of a year totaling _24000_ NBTC.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<neobitcoin_address\> \<monthly_payment_neobitcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` NBTC.
 ...
 * vote-many          - Vote on a NeoBitcoin initiative
 * vote-alias         - Vote on a NeoBitcoin initiative
 * vote               - Vote on a NeoBitcoin initiative/budget
